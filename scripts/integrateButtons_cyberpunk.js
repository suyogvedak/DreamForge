// scripts/integrateButtons_cyberpunk.js
...
function inject3D() {
  const charBtn = Array.from(neonButtons).find(b => b.dataset.type === 'character');
  const weapBtn = Array.from(neonButtons).find(b => b.dataset.type === 'weapon');
  ...
  if (charBtn && !document.getElementById('gen3d-character')) {
    const b = create3D('gen3d-character');
    b.addEventListener('click', () => dfToast('3D Character generation not yet wired.'));
    charBtn.parentElement?.appendChild(b);
  }

  if (weapBtn && !document.getElementById('gen3d-weapon')) {
    const b = create3D('gen3d-weapon');
    b.addEventListener('click', () => dfToast('3D Weapon generation not yet wired.'));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', async () => {
      const payload = saveBtn._payload;
