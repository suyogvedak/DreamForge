// scripts/integrateButtons_fantasy.js
const THEME = 'fantasy';
...
const actionButtons = document.querySelectorAll('.fantasy-action'); // your page likely has buttons with this class
...
function inject3D() {
  ...
  if (charBtn && !document.getElementById('gen3d-character')) {
    const b = create3D('gen3d-character-fantasy');
    b.addEventListener('click', () => dfToast('3D Character generation not yet wired for Fantasy.'));
    charBtn.parentElement?.appendChild(b);
  }

  if (weapBtn && !document.getElementById('gen3d-weapon')) {
    const b = create3D('gen3d-weapon-fantasy');
    b.addEventListener('click', () => dfToast('3D Weapon generation not yet wired for Fantasy.'));
